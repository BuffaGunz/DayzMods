...
class RuckLocationFinder
{
	...
	static string MSG_NEAR = "Supply drop inbound - scheduled to land in %1.";
	static string MSG_AWAY = "Supply drop inbound - scheduled to land %1 %2 of %3.";

	protected static ref array<ref RuckTown> s_Towns;
	protected static bool s_Built;

	static string BuildAirdropMessage(vector dropPos)
	{
		EnsureBuilt();

		float dist;
		RuckTown town = FindNearestTown(dropPos, dist);
		if (!town)
			return "Supply drop inbound — scheduled to land at the designated coordinates.";

		float effectiveRadius = town.radius + RADIUS_PADDING;
		if (dist <= effectiveRadius)
			return string.Format(MSG_NEAR, town.name);

		float brg = BearingDeg(town.pos, dropPos);
	...
	{
		s_Towns = new array<ref RuckTown>;
	...
	protected static RuckTown FindNearestTown(vector p, out float distMeters)
	{
		RuckTown best; float bestSq = 1e20;
		foreach (RuckTown t : s_Towns)
		{
