class Ikarus extends CarScript
{
	...
	{
		return CarLightBase.Cast(ScriptedLightBase.CreateLight(IkarusFrontLight));
	}
	
	// Override for car-specific light type
	override CarRearLightBase CreateRearLight()
	{
		return CarRearLightBase.Cast(ScriptedLightBase.CreateLight(IkarusRearLight));
	}
	...
	override bool CrewCanGetThrough( int posIdx )
	{
		...
		{
			case 0:
				if ( GetCarDoorsState( "BusLeftDoors_1" ) == CarDoorState.DOORS_CLOSED )
					return false;
			...
			case 1:
				if ( GetCarDoorsState( "BusRightDoors_1" ) == CarDoorState.DOORS_CLOSED )
					return false;
			...
			case 2:
				if ( GetCarDoorsState( "BusRightDoors_2" ) == CarDoorState.DOORS_CLOSED )
					return false;
	...
	override float OnSound( CarSoundCtrl ctrl, float oldValue )
	{
		switch ( ctrl )
		{
			case CarSoundCtrl.DOORS:
				...
				//-----
				if ( GetCarDoorsState( "BusLeftDoors_2" ) == CarDoorState.DOORS_CLOSED )
					newValue += 0.25;
				...
				//-----
				if ( GetCarDoorsState( "BusLeftDoors_3" ) == CarDoorState.DOORS_CLOSED )
					newValue += 0.25;

				//-----
				if ( GetCarDoorsState( "BusRightDoors_3" ) == CarDoorState.DOORS_CLOSED )
					newValue += 0.125;

				//-----
				if ( GetCarDoorsState( "BusHood" ) == CarDoorState.DOORS_CLOSED )
					newValue += 0.125;
	...
	override void OnDebugSpawn()
	{
		...
		GetInventory().CreateInInventory("IkarusWheel");
		GetInventory().CreateInInventory("IkarusWheel");
		GetInventory().CreateInInventory("ikaruswheel_rear" );
		GetInventory().CreateInInventory("ikaruswheel_rear");

		GetInventory().CreateInInventory("BusLeftDoors_1");
		GetInventory().CreateInInventory("BusLeftDoors_2");
		GetInventory().CreateInInventory("BusLeftDoors_3");
		GetInventory().CreateInInventory("BusRightDoors_1");
		GetInventory().CreateInInventory("BusRightDoors_2");
		GetInventory().CreateInInventory("BusRightDoors_3");
		GetInventory().CreateInInventory("BusHood");
